---
icon: material/clipboard-list-outline
---

# The Offer

This is how an engagement works, from first contact to running infrastructure.
Each step is agreed before the next one begins — you're not committing to
anything beyond where you currently are.

---

## :material-calendar-outline: Free call

Before anything else, we talk. A 30-minute conversation where we look at your
current setup and you tell me what's driving the pain — the bill, the
complexity, an inherited mess, or something else entirely.

I'll tell you honestly whether I think a migration makes sense for your situation.
If it doesn't — if your stack is too specific, or simply not worth it —
I'll say so and we part ways without obligation. No pitch, no follow-up pressure.

[Book via Calendly :octicons-arrow-right-24:](https://calendly.com/roman-hypha/call-with-roman){ .md-button .md-button--primary }

---

## :material-file-search-outline: Infrastructure audit

If the free call suggests a migration is worth exploring, the next step is
a paid audit. I review your setup in detail and produce a written report — a
clear picture of what you're running, what it actually costs, and what a
better alternative looks like. Take it and act on it however you like, with
whoever you like.

**What I look at:**

- [x] Your current architecture — what's running, where, and why
- [x] Your monthly bill broken down by service
- [x] What you actually need vs what you're paying for

**What you receive:**

- [x] A plain-language description of your current infrastructure and its real costs
- [x] A proposed architecture on FreeBSD and bare metal
- [x] Estimated monthly server costs after migration, with specs and provider options
- [x] A fixed implementation price if you want to proceed
- [x] An honest assessment of migration complexity and timeline

Takes approximately one week from the initial conversation to delivery.
The audit fee is deducted from implementation if you proceed.

!!! quote ""
    **From £750**

---

## :material-server: Implementation

If you want to proceed, we agree on scope and price before any work begins.

**What's included in every implementation:**

- [x] Bare metal server provisioning — you hold the contracts directly
- [x] FreeBSD installation and hardening
- [x] Your services running in jails
- [x] Networking, firewall, and reverse proxy configuration
- [x] ZFS-based automated backups with tested restore procedure
- [x] Blue/green deployment for zero-downtime releases
- [x] Monitoring and alerting
- [x] Daily stats and health summaries by email
- [x] Full documentation of every decision made
- [x] One month of post-launch support included

Servers are billed separately and directly to you.

!!! quote ""
    **From £3,000**

---

## :material-wrench-outline: Monthly support

Included for the first month after implementation. After that, optional.
Everything is documented and yours to operate independently. If you'd
rather have someone watching over it — keeping it updated, patched, and
running cleanly — that's what this is.

**What's included:**

- [x] OS and package updates applied and tested
- [x] Security patches within 24 hours of release
- [x] Monitoring review and alert response during business hours
- [x] Direct line

Rolling monthly contract. No minimum term — cancel with 30 days notice.

!!! quote ""
    **From £300 per month**

---

## :material-handshake-outline: Bespoke engagement

Something that doesn't fit a fixed scope — code review, performance analysis,
architecture consulting, or a second opinion on a hard problem. Custom scope,
custom price, direct conversation.

[Get in touch :octicons-arrow-right-24:](mailto:dev@hyphatech.io){ .md-button }

---

## :material-currency-gbp: A note on pricing

All prices are fixed. You will not receive an invoice you didn't expect.
