...
def ssh(state: State, settings: Settings, jail_name: str | None = None, cmd: list[str] | None = None) -> None:
    ...
    if jail_name:
        ...
        command = jail_ssh_cmd(args=cmd or [], jail_ip=jail_ip, tty=True, **ssh_kw)
    else:
        command = ssh_cmd(args=cmd or [], tty=True, **ssh_kw)
